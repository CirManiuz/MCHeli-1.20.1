...
public class ExpRuleFactory {
    ...
    public static Rule getDefaultRule() {
        return ExpRuleFactory.getInstance().getRule();
    }
    ...
    protected void initFuncArgRule(ShareRuleValue share) {
        ...
        boolean match = false;
        block0: for (int i = 0; i < a_opes.length; ++i) {
            for (int j = 0; j < t_opes.length; ++j) {
                if (!a_opes[i].equals(t_opes[j])) continue;
                match = true;
                break block0;
            }
        }
        argRule.nextRule = match ? this.topRule.nextRule : this.topRule;
    ...
    protected AbstractRule createIfRule(ShareRuleValue share) {
        ...
        me.cond = this.createIfExpression();
        me.addExpression(me.cond);
        return me;
    }
    ...
    protected AbstractRule createCol1AfterRule(ShareRuleValue share) {
        ...
        me.func = this.createFunctionExpression();
        me.addExpression(me.func);
        me.array = this.createArrayExpression();
        me.addExpression(me.array);
        me.addExpression(this.createIncAfterExpression());
        me.addExpression(this.createDecAfterExpression());
        me.field = this.createFieldExpression();
        me.addExpression(me.field);
        return me;
    }
